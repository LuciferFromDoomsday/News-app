...
class getMovie: ObservableObject {
    ...
    init () {
        ...
        let source  = "https://api.themoviedb.org/3/movie/popular?api_key=82bb3b5cf7b870891c07d7d362fe888a&language=en-US&page=\(getMovie.page)"
    ...
    func addExtra(){
        getMovie.page += 1
        let source  = "https://api.themoviedb.org/3/movie/popular?api_key=82bb3b5cf7b870891c07d7d362fe888a&language=en-US&page=1\(getMovie.page)"
        let url = URL(string: source)!
               let session = URLSession(configuration: .default)
        session.dataTask(with: url){
                   (data , _ , error) in
